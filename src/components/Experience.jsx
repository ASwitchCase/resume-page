import React from 'react'

const Experience = () => {
  return (
    <div>
      <div className='content-container'>
        <h1>Experience</h1>
        <h2>Contract Technical Consultant for Metamechanics</h2>
        <h3>2023 - Present</h3>
        <ul>
          <li>
            <p>Helped remove malware from Company Website, and made edits to site using WordPress.</p>
          </li>
        </ul>

        <h2>Student Clerk, Visual Arts Building, NJCU</h2>
        <h3>2019 - 2023</h3>
        <ul>
          <li>
            <p>Developed a database application for student locker assignments. Assisted with office work such as filing, creating schedules, and populating spreadsheets.</p>
          </li>
        </ul>
        

        <h2>Intern at the Office of innovation, City Hall</h2>
        <h3>Summer of 2019</h3>

        <ul>
          <li>
            <p>Conducted research on green spaces in Jersey City New Jersey. Used WordPress to make edits and write blog entries on innovatejc.org.</p>
          </li>
        </ul>
        

        <h2>Computer Science Tutor</h2>
        <h3>2018 - Present</h3>

        <h1>Volunteer Work</h1>
        <h2>Audio Video Technician, For religious organization</h2>
        <h3>2020 - Present</h3>

        <ul>
          <li>
          <p>Provided support to the AV team and helped broadcast meetings through video conferencing software like Zoom.</p>
          </li>
        </ul>
        
      </div>
    </div>
  )
}

export default Experience
